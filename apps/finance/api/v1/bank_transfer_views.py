import json

from apps.authorization.services import (
    AuthorizationService,
)
from apps.core.api.decorators import (
    api_login_required,
    api_rate_limit,
)
from apps.core.services.api_query_pipeline_service import (
    APIQueryPipelineError,
    APIQueryPipelineService,
)
from apps.core.services.api_response_service import (
    APIResponseService,
)
from apps.finance.api.v1.serializers import (
    BankTransferAPISerializer,
)
from apps.finance.repositories.bank_transfer_repository import (
    BankTransferRepository,
)
from apps.finance.services.bank_transfer_api_service import (
    BankTransferAPIService,
    BankTransferAPIStateError,
    BankTransferAPIValidationError,
)


def _json_body(
    request,
):
    content_type = (
        request.content_type
        or
        ""
    ).lower()

    if (
        "application/json"
        not in content_type
    ):
        return (
            None,
            APIResponseService
            .validation_error(
                message=(
                    "Content-Type must be "
                    "application/json."
                ),
                details={
                    "content_type": [
                        (
                            "Send the request body "
                            "as JSON."
                        ),
                    ],
                },
                request=request,
            ),
        )

    try:
        return (
            json.loads(
                request.body
                or
                b"{}"
            ),
            None,
        )

    except (
        json.JSONDecodeError,
        UnicodeDecodeError,
    ):
        return (
            None,
            APIResponseService
            .validation_error(
                message="Malformed JSON body.",
                details={
                    "body": [
                        (
                            "Request body must "
                            "contain valid JSON."
                        ),
                    ],
                },
                request=request,
            ),
        )


@api_login_required
@api_rate_limit(
    scope="bank_transfers.collection",
    limit=120,
    window_seconds=60,
)
def bank_transfer_collection_api(
    request,
):
    if request.method == "GET":
        if not (
            AuthorizationService
            .has_permission(
                request.api_user,
                "bank_transfers.read",
            )
        ):
            return (
                APIResponseService
                .forbidden(
                    message="Permission denied.",
                    request=request,
                )
            )

        queryset = (
            BankTransferRepository
            .queryset_for_organization(
                organization=(
                    request.api_organization
                ),
            )
        )

        try:
            pipeline_result = (
                APIQueryPipelineService
                .execute(
                    queryset,
                    request,
                    allowed_filters={
                        "source_account_id": {
                            "field":
                                "source_account",
                            "parser":
                                "object_id",
                        },
                        "destination_account_id": {
                            "field": (
                                "destination_account"
                            ),
                            "parser":
                                "object_id",
                        },
                        "status": {
                            "field":
                                "status",
                            "parser":
                                "string",
                        },
                    },
                    search_fields=[
                        "transfer_number",
                        "reference",
                        "notes",
                    ],
                    allowed_sort_fields={
                        "transfer_number":
                            "transfer_number",
                        "transfer_date":
                            "transfer_date",
                        "amount":
                            "amount",
                        "status":
                            "status",
                        "created_at":
                            "created_at",
                        "updated_at":
                            "updated_at",
                    },
                    default_sort=[
                        "-transfer_date",
                    ],
                    stable_sort_field="id",
                    default_page_size=25,
                    maximum_page_size=100,
                )
            )

        except APIQueryPipelineError as exc:
            return (
                APIResponseService
                .validation_error(
                    message=exc.message,
                    details={
                        "component":
                            exc.component,
                        "fields":
                            exc.details,
                    },
                    request=request,
                )
            )

        return (
            APIResponseService
            .success(
                data={
                    "bank_transfers": (
                        BankTransferAPISerializer
                        .serialize_many(
                            pipeline_result[
                                "items"
                            ]
                        )
                    ),
                    "pagination":
                        pipeline_result[
                            "pagination"
                        ],
                    "query":
                        pipeline_result[
                            "query"
                        ],
                },
                request=request,
            )
        )

    if request.method == "POST":
        if not (
            AuthorizationService
            .has_permission(
                request.api_user,
                "bank_transfers.create",
            )
        ):
            return (
                APIResponseService
                .forbidden(
                    message="Permission denied.",
                    request=request,
                )
            )

        payload, error_response = (
            _json_body(
                request
            )
        )

        if error_response:
            return error_response

        try:
            transfer = (
                BankTransferAPIService
                .create_transfer(
                    user=request.api_user,
                    organization=(
                        request.api_organization
                    ),
                    payload=payload,
                )
            )

        except (
            BankTransferAPIValidationError
        ) as exc:
            return (
                APIResponseService
                .validation_error(
                    message=exc.message,
                    details=exc.details,
                    request=request,
                )
            )

        except (
            BankTransferAPIStateError
        ) as exc:
            return (
                APIResponseService
                .unprocessable_entity(
                    message=exc.message,
                    details=exc.details,
                    request=request,
                )
            )

        except PermissionError:
            return (
                APIResponseService
                .forbidden(
                    message="Permission denied.",
                    request=request,
                )
            )

        return (
            APIResponseService
            .success(
                data={
                    "bank_transfer": (
                        BankTransferAPISerializer
                        .serialize_detail(
                            transfer
                        )
                    ),
                },
                message=(
                    "Bank transfer created "
                    "successfully."
                ),
                status=201,
                request=request,
            )
        )

    return (
        APIResponseService
        .method_not_allowed(
            message=(
                "Use GET to list bank transfers "
                "or POST to create one."
            ),
            request=request,
        )
    )


@api_login_required
@api_rate_limit(
    scope="bank_transfers.detail",
    limit=120,
    window_seconds=60,
)
def bank_transfer_detail_api(
    request,
    transfer_id,
):
    if request.method != "GET":
        return (
            APIResponseService
            .method_not_allowed(
                message=(
                    "Use GET to retrieve a "
                    "bank transfer."
                ),
                request=request,
            )
        )

    if not (
        AuthorizationService
        .has_permission(
            request.api_user,
            "bank_transfers.read",
        )
    ):
        return (
            APIResponseService
            .forbidden(
                message="Permission denied.",
                request=request,
            )
        )

    try:
        transfer = (
            BankTransferAPIService
            .get_transfer(
                organization=(
                    request.api_organization
                ),
                transfer_id=transfer_id,
            )
        )

    except (
        BankTransferAPIValidationError
    ) as exc:
        return (
            APIResponseService
            .validation_error(
                message=exc.message,
                details=exc.details,
                request=request,
            )
        )

    except LookupError:
        return (
            APIResponseService
            .not_found(
                message="Bank transfer not found.",
                request=request,
            )
        )

    return (
        APIResponseService
        .success(
            data={
                "bank_transfer": (
                    BankTransferAPISerializer
                    .serialize_detail(
                        transfer
                    )
                ),
            },
            request=request,
        )
    )


@api_login_required
@api_rate_limit(
    scope="bank_transfers.post",
    limit=60,
    window_seconds=60,
)
def bank_transfer_post_api(
    request,
    transfer_id,
):
    if request.method != "POST":
        return (
            APIResponseService
            .method_not_allowed(
                message=(
                    "Use POST to post a "
                    "bank transfer."
                ),
                request=request,
            )
        )

    if not (
        AuthorizationService
        .has_permission(
            request.api_user,
            "bank_transfers.post",
        )
    ):
        return (
            APIResponseService
            .forbidden(
                message="Permission denied.",
                request=request,
            )
        )

    try:
        transfer = (
            BankTransferAPIService
            .post_transfer(
                user=request.api_user,
                organization=(
                    request.api_organization
                ),
                transfer_id=transfer_id,
            )
        )

    except (
        BankTransferAPIValidationError
    ) as exc:
        return (
            APIResponseService
            .validation_error(
                message=exc.message,
                details=exc.details,
                request=request,
            )
        )

    except LookupError:
        return (
            APIResponseService
            .not_found(
                message="Bank transfer not found.",
                request=request,
            )
        )

    except (
        BankTransferAPIStateError
    ) as exc:
        return (
            APIResponseService
            .unprocessable_entity(
                message=exc.message,
                details=exc.details,
                request=request,
            )
        )

    except PermissionError:
        return (
            APIResponseService
            .forbidden(
                message="Permission denied.",
                request=request,
            )
        )

    return (
        APIResponseService
        .success(
            data={
                "bank_transfer": (
                    BankTransferAPISerializer
                    .serialize_detail(
                        transfer
                    )
                ),
            },
            message=(
                "Bank transfer posted "
                "successfully."
            ),
            request=request,
        )
    )


@api_login_required
@api_rate_limit(
    scope="bank_transfers.cancel",
    limit=60,
    window_seconds=60,
)
def bank_transfer_cancel_api(
    request,
    transfer_id,
):
    if request.method != "POST":
        return (
            APIResponseService
            .method_not_allowed(
                message=(
                    "Use POST to cancel a "
                    "bank transfer."
                ),
                request=request,
            )
        )

    if not (
        AuthorizationService
        .has_permission(
            request.api_user,
            "bank_transfers.cancel",
        )
    ):
        return (
            APIResponseService
            .forbidden(
                message="Permission denied.",
                request=request,
            )
        )

    try:
        transfer = (
            BankTransferAPIService
            .cancel_transfer(
                user=request.api_user,
                organization=(
                    request.api_organization
                ),
                transfer_id=transfer_id,
            )
        )

    except (
        BankTransferAPIValidationError
    ) as exc:
        return (
            APIResponseService
            .validation_error(
                message=exc.message,
                details=exc.details,
                request=request,
            )
        )

    except LookupError:
        return (
            APIResponseService
            .not_found(
                message="Bank transfer not found.",
                request=request,
            )
        )

    except (
        BankTransferAPIStateError
    ) as exc:
        return (
            APIResponseService
            .unprocessable_entity(
                message=exc.message,
                details=exc.details,
                request=request,
            )
        )

    except PermissionError:
        return (
            APIResponseService
            .forbidden(
                message="Permission denied.",
                request=request,
            )
        )

    return (
        APIResponseService
        .success(
            data={
                "bank_transfer": (
                    BankTransferAPISerializer
                    .serialize_detail(
                        transfer
                    )
                ),
            },
            message=(
                "Bank transfer cancelled "
                "successfully."
            ),
            request=request,
        )
    )