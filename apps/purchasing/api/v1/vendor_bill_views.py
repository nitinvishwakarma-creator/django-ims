import json

from apps.authorization.services import (
    AuthorizationService,
)
from apps.core.api.decorators import (
    api_login_required,
    api_rate_limit,
)
from apps.core.services.api_query_pipeline_service import (
    APIQueryPipelineError,
    APIQueryPipelineService,
)
from apps.core.services.api_response_service import (
    APIResponseService,
)
from apps.purchasing.api.v1.serializers import (
    PurchasingBankAccountAPISerializer,
    SupplierPaymentAPISerializer,
    VendorBillAPISerializer,
)
from apps.purchasing.repositories.vendor_bill_repository import (
    VendorBillRepository,
)
from apps.purchasing.services.vendor_bill_api_service import (
    VendorBillAPIService,
    VendorBillAPIStateError,
    VendorBillAPIValidationError,
)


def _json_body(
    request,
):
    content_type = (
        request.content_type
        or
        ""
    ).lower()

    if (
        "application/json"
        not in content_type
    ):
        return (
            None,
            APIResponseService
            .validation_error(
                message=(
                    "Content-Type must be "
                    "application/json."
                ),
                details={
                    "content_type": [
                        (
                            "Send the request body "
                            "as JSON."
                        ),
                    ],
                },
                request=request,
            ),
        )

    try:
        return (
            json.loads(
                request.body
                or
                b"{}"
            ),
            None,
        )

    except (
        json.JSONDecodeError,
        UnicodeDecodeError,
    ):
        return (
            None,
            APIResponseService
            .validation_error(
                message="Malformed JSON body.",
                details={
                    "body": [
                        (
                            "Request body must "
                            "contain valid JSON."
                        ),
                    ],
                },
                request=request,
            ),
        )


@api_login_required
@api_rate_limit(
    scope="vendor_bills.collection",
    limit=120,
    window_seconds=60,
)
def vendor_bill_collection_api(
    request,
):
    if request.method == "GET":

        if not AuthorizationService.has_permission(
            request.api_user,
            "bills.read",
        ):
            return (
                APIResponseService
                .forbidden(
                    message="Permission denied.",
                    request=request,
                )
            )

        queryset = (
            VendorBillRepository
            .queryset_for_organization(
                organization=(
                    request.api_organization
                ),
            )
        )

        try:
            pipeline_result = (
                APIQueryPipelineService
                .execute(
                    queryset,
                    request,
                    allowed_filters={
                        "supplier_id": {
                            "field":
                                "supplier",
                            "parser":
                                "object_id",
                        },
                        "purchase_order_id": {
                            "field":
                                "purchase_order",
                            "parser":
                                "object_id",
                        },
                        "status": {
                            "field":
                                "status",
                            "parser":
                                "string",
                        },
                    },
                    search_fields=[
                        "bill_number",
                        (
                            "supplier_invoice_"
                            "number"
                        ),
                        "supplier_name",
                        "supplier_gstin",
                    ],
                    allowed_sort_fields={
                        "bill_number":
                            "bill_number",
                        "bill_date":
                            "bill_date",
                        "due_date":
                            "due_date",
                        "status":
                            "status",
                        "total_amount":
                            "total_amount",
                        "balance_due":
                            "balance_due",
                        "created_at":
                            "created_at",
                        "updated_at":
                            "updated_at",
                    },
                    default_sort=[
                        "-created_at",
                    ],
                    stable_sort_field="id",
                    default_page_size=25,
                    maximum_page_size=100,
                )
            )

        except APIQueryPipelineError as exc:
            return (
                APIResponseService
                .validation_error(
                    message=exc.message,
                    details={
                        "component":
                            exc.component,
                        "fields":
                            exc.details,
                    },
                    request=request,
                )
            )

        return (
            APIResponseService
            .success(
                data={
                    "vendor_bills": (
                        VendorBillAPISerializer
                        .serialize_many(
                            pipeline_result[
                                "items"
                            ]
                        )
                    ),
                    "pagination":
                        pipeline_result[
                            "pagination"
                        ],
                    "query":
                        pipeline_result[
                            "query"
                        ],
                },
                message=(
                    "Vendor bills retrieved "
                    "successfully."
                ),
                request=request,
            )
        )

    if request.method == "POST":

        if not AuthorizationService.has_permission(
            request.api_user,
            "bills.create",
        ):
            return (
                APIResponseService
                .forbidden(
                    message="Permission denied.",
                    request=request,
                )
            )

        payload, error_response = (
            _json_body(request)
        )

        if error_response:
            return error_response

        try:
            bill = (
                VendorBillAPIService
                .create_vendor_bill(
                    user=request.api_user,
                    organization=(
                        request.api_organization
                    ),
                    payload=payload,
                )
            )

        except VendorBillAPIValidationError as exc:
            return (
                APIResponseService
                .validation_error(
                    message=exc.message,
                    details=exc.details,
                    request=request,
                )
            )

        except LookupError as exc:
            return (
                APIResponseService
                .not_found(
                    message=str(exc),
                    request=request,
                )
            )

        except VendorBillAPIStateError as exc:
            return (
                APIResponseService
                .unprocessable_entity(
                    message=exc.message,
                    details=exc.details,
                    request=request,
                )
            )

        except PermissionError:
            return (
                APIResponseService
                .forbidden(
                    message="Permission denied.",
                    request=request,
                )
            )

        return (
            APIResponseService
            .success(
                data={
                    "vendor_bill": (
                        VendorBillAPISerializer
                        .serialize_detail(
                            bill
                        )
                    ),
                },
                message=(
                    "Vendor bill created "
                    "successfully."
                ),
                status=201,
                request=request,
            )
        )

    return (
        APIResponseService
        .method_not_allowed(
            message=(
                "Use GET or POST for "
                "vendor bills."
            ),
            request=request,
        )
    )


@api_login_required
@api_rate_limit(
    scope="vendor_bills.detail",
    limit=120,
    window_seconds=60,
)
def vendor_bill_detail_api(
    request,
    bill_id,
):
    if request.method != "GET":
        return (
            APIResponseService
            .method_not_allowed(
                message=(
                    "Use GET to retrieve "
                    "a vendor bill."
                ),
                request=request,
            )
        )

    if not AuthorizationService.has_permission(
        request.api_user,
        "bills.read",
    ):
        return (
            APIResponseService
            .forbidden(
                message="Permission denied.",
                request=request,
            )
        )

    try:
        bill = (
            VendorBillAPIService
            .get_vendor_bill(
                organization=(
                    request.api_organization
                ),
                bill_id=bill_id,
            )
        )

    except VendorBillAPIValidationError as exc:
        return (
            APIResponseService
            .validation_error(
                message=exc.message,
                details=exc.details,
                request=request,
            )
        )

    except LookupError:
        return (
            APIResponseService
            .not_found(
                message="Vendor bill not found.",
                request=request,
            )
        )

    return (
        APIResponseService
        .success(
            data={
                "vendor_bill": (
                    VendorBillAPISerializer
                    .serialize_detail(
                        bill
                    )
                ),
            },
            message=(
                "Vendor bill retrieved "
                "successfully."
            ),
            request=request,
        )
    )

@api_login_required
@api_rate_limit(
    scope="vendor_bills.post",
    limit=60,
    window_seconds=60,
)
def vendor_bill_post_api(
    request,
    bill_id,
):
    if request.method != "POST":
        return (
            APIResponseService
            .method_not_allowed(
                message=(
                    "Use POST to post "
                    "a vendor bill."
                ),
                request=request,
            )
        )

    if not AuthorizationService.has_permission(
        request.api_user,
        "bills.post",
    ):
        return (
            APIResponseService
            .forbidden(
                message="Permission denied.",
                request=request,
            )
        )

    try:
        bill = (
            VendorBillAPIService
            .post_vendor_bill(
                user=request.api_user,
                organization=(
                    request.api_organization
                ),
                bill_id=bill_id,
            )
        )

    except VendorBillAPIValidationError as exc:
        return (
            APIResponseService
            .validation_error(
                message=exc.message,
                details=exc.details,
                request=request,
            )
        )

    except LookupError:
        return (
            APIResponseService
            .not_found(
                message="Vendor bill not found.",
                request=request,
            )
        )

    except VendorBillAPIStateError as exc:
        return (
            APIResponseService
            .unprocessable_entity(
                message=exc.message,
                details=exc.details,
                request=request,
            )
        )

    except PermissionError:
        return (
            APIResponseService
            .forbidden(
                message="Permission denied.",
                request=request,
            )
        )

    return (
        APIResponseService
        .success(
            data={
                "vendor_bill": (
                    VendorBillAPISerializer
                    .serialize_detail(
                        bill
                    )
                ),
            },
            message=(
                "Vendor bill posted "
                "successfully."
            ),
            request=request,
        )
    )


@api_login_required
@api_rate_limit(
    scope="vendor_bills.cancel",
    limit=60,
    window_seconds=60,
)
def vendor_bill_cancel_api(
    request,
    bill_id,
):
    if request.method != "POST":
        return (
            APIResponseService
            .method_not_allowed(
                message=(
                    "Use POST to cancel "
                    "a vendor bill."
                ),
                request=request,
            )
        )

    if not AuthorizationService.has_permission(
        request.api_user,
        "bills.cancel",
    ):
        return (
            APIResponseService
            .forbidden(
                message="Permission denied.",
                request=request,
            )
        )

    try:
        bill = (
            VendorBillAPIService
            .cancel_vendor_bill(
                user=request.api_user,
                organization=(
                    request.api_organization
                ),
                bill_id=bill_id,
            )
        )

    except VendorBillAPIValidationError as exc:
        return (
            APIResponseService
            .validation_error(
                message=exc.message,
                details=exc.details,
                request=request,
            )
        )

    except LookupError:
        return (
            APIResponseService
            .not_found(
                message="Vendor bill not found.",
                request=request,
            )
        )

    except VendorBillAPIStateError as exc:
        return (
            APIResponseService
            .unprocessable_entity(
                message=exc.message,
                details=exc.details,
                request=request,
            )
        )

    except PermissionError:
        return (
            APIResponseService
            .forbidden(
                message="Permission denied.",
                request=request,
            )
        )

    return (
        APIResponseService
        .success(
            data={
                "vendor_bill": (
                    VendorBillAPISerializer
                    .serialize_detail(
                        bill
                    )
                ),
            },
            message=(
                "Vendor bill cancelled "
                "successfully."
            ),
            request=request,
        )
    )


@api_login_required
@api_rate_limit(
    scope="vendor_bills.record_payment",
    limit=60,
    window_seconds=60,
)
def vendor_bill_record_payment_api(
    request,
    bill_id,
):
    if request.method != "POST":
        return (
            APIResponseService
            .method_not_allowed(
                message=(
                    "Use POST to record "
                    "a vendor bill payment."
                ),
                request=request,
            )
        )

    if not AuthorizationService.has_permission(
        request.api_user,
        "bills.record_payment",
    ):
        return (
            APIResponseService
            .forbidden(
                message="Permission denied.",
                request=request,
            )
        )

    payload, error_response = (
        _json_body(request)
    )

    if error_response:
        return error_response

    try:
        result = (
            VendorBillAPIService
            .record_payment(
                user=request.api_user,
                organization=(
                    request.api_organization
                ),
                bill_id=bill_id,
                payload=payload,
            )
        )

    except VendorBillAPIValidationError as exc:
        return (
            APIResponseService
            .validation_error(
                message=exc.message,
                details=exc.details,
                request=request,
            )
        )

    except LookupError as exc:
        return (
            APIResponseService
            .not_found(
                message=str(exc),
                request=request,
            )
        )

    except VendorBillAPIStateError as exc:
        return (
            APIResponseService
            .unprocessable_entity(
                message=exc.message,
                details=exc.details,
                request=request,
            )
        )

    except PermissionError:
        return (
            APIResponseService
            .forbidden(
                message="Permission denied.",
                request=request,
            )
        )

    return (
        APIResponseService
        .success(
            data={
                "vendor_bill": (
                    VendorBillAPISerializer
                    .serialize_detail(
                        result[
                            "vendor_bill"
                        ]
                    )
                ),
                "payment": (
                    SupplierPaymentAPISerializer
                    .serialize_detail(
                        result[
                            "payment"
                        ]
                    )
                ),
            },
            message=(
                "Vendor bill payment recorded "
                "successfully."
            ),
            status=201,
            request=request,
        )
    )


@api_login_required
@api_rate_limit(
    scope="vendor_bills.bank_accounts",
    limit=120,
    window_seconds=60,
)
def vendor_bill_bank_account_list_api(
    request,
):
    if request.method != "GET":
        return (
            APIResponseService
            .method_not_allowed(
                message=(
                    "Use GET to retrieve "
                    "bank accounts."
                ),
                request=request,
            )
        )

    if not AuthorizationService.has_permission(
        request.api_user,
        "bills.record_payment",
    ):
        return (
            APIResponseService
            .forbidden(
                message="Permission denied.",
                request=request,
            )
        )

    bank_accounts = (
        VendorBillAPIService
        .list_active_bank_accounts(
            organization=(
                request.api_organization
            ),
        )
    )

    return (
        APIResponseService
        .success(
            data={
                "bank_accounts": (
                    PurchasingBankAccountAPISerializer
                    .serialize_many(
                        bank_accounts
                    )
                ),
            },
            message=(
                "Bank accounts retrieved "
                "successfully."
            ),
            request=request,
        )
    )


@api_login_required
@api_rate_limit(
    scope="accounts_payable.summary",
    limit=120,
    window_seconds=60,
)
def accounts_payable_api(
    request,
):
    if request.method != "GET":
        return (
            APIResponseService
            .method_not_allowed(
                message=(
                    "Use GET to retrieve "
                    "accounts payable."
                ),
                request=request,
            )
        )

    if not AuthorizationService.has_permission(
        request.api_user,
        "bills.read",
    ):
        return (
            APIResponseService
            .forbidden(
                message="Permission denied.",
                request=request,
            )
        )

    bills = list(
        VendorBillAPIService
        .list_outstanding(
            organization=(
                request.api_organization
            ),
        )
    )

    total_outstanding = sum(
        (
            bill.balance_due
            for bill in bills
        ),
        start=0,
    )

    supplier_ids = {
        str(
            bill.supplier.id
        )
        for bill in bills
        if bill.supplier
    }

    return (
        APIResponseService
        .success(
            data={
                "accounts_payable": {
                    "bill_count":
                        len(bills),
                    "supplier_count":
                        len(
                            supplier_ids
                        ),
                    "total_outstanding":
                        str(
                            total_outstanding
                        ),
                    "vendor_bills": (
                        VendorBillAPISerializer
                        .serialize_many(
                            bills
                        )
                    ),
                },
            },
            message=(
                "Accounts payable retrieved "
                "successfully."
            ),
            request=request,
        )
    )